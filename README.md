...
**V0–V2 completed; V3–V5 not attested; no real fit has ever been run.** This is the status pinned at the frozen code identity (`test_active_authority_and_instructions_are_aligned`, inside the freeze perimeter); since 24 September 2026 V3 is attested, see [HANDOFF](docs/HANDOFF.md). Implemented and verified on synthetic and analytical fixtures: the corpus reader and encoding, the frozen CTW, sampling with the contract RNG and the shuffle, the four losses with G and Q, diagnostics, R1, atomic persistence with resume, the report tables and the five figures. The real corpus has been read and encoded once (V1), and its nine artifacts are reproducible byte for byte. The campaign, the final contrasts and the results belong to V4–V5 and do not exist. Work stops for review before V4.
