...
2. **Codice v2.1 nei moduli vivi.** Vengono rimossi:
   ...
   Gli stadi ritirati non esistono più: è un vincolo più forte del rifiuto di eseguirli (§13.2, «ritirare CLI»). `check_output_locations`, `staged_inputs` e `verify_inputs_unchanged`, con le loro dipendenze `_conllu_paths` e `CANONICAL_DATA_ROOT`, passano da `legacy_audit` a `corpus_run` senza modifiche; l'identità è verificata con `ast.dump`.
