...
Neither the realignment nor the rename changed the contract or the data: deposit, configuration, `uv.lock`, provenance and raw data are unchanged, and the V1 corpus has been regenerated byte for byte in its nine artifacts after each. Both changed the code identity before any V2 or V3 evidence was published; the pre-V3 freeze declared on 22 September is closed by V3: run identity `1563452a…` is frozen.
