...
The owner asked that the finished results be visible and the repository readable by anyone who opens it. On `9814871` the annotated tag `hexis31-v5-evidence` fixes the code and perimeter of the evidence. `40c8528` tracks the delivered corpus, the readable layer of the final campaign and of its regeneration, and the V3–V5 logs, byte for byte (83 files, 14 MB), with the two checks previously kept outside the repository; `pyproject.toml` changes only in its sdist list. The complete runs are two archives of the GitHub Release `hexis31-v5-evidence`, with the SHA-256 recorded in V3-007 and in [results/README.md](../results/README.md). A second commit moves the statement of results to [RESULTS.md](RESULTS.md), rewrites the README as the entry page, adds `results/README.md`, turns the index into a reading guide and adds V3-007.

Checks, before and after the first commit, verbatim: `validate_run` accepted `v3-seed0-v3006` (931 artifacts), `v3-seed0-v3006-regeneration` (73) and the corpus `v1`; `pytest` and `pytest -m v31` gave 431 passed with identical collections of 431 IDs; `src/hormathos`, `tests/`, `config/`, `uv.lock`, `conftest.py`, the deposit and `archive/` are unchanged from `9814871`. Every run extracted from the two archives passes `validate_run`: the final campaign, its regeneration and `v1`; `v1-development`, `v1-reproduction`, `v3-seed0`, `v3-seed0-regeneration`, `v3-seed0-r4-r6` and `v3-seed0-r4-r6-regeneration` (73 artifacts each).
...
`_code_identity()` covers all of `src/hormathos`; the V2 evidence (`validation_run.context`) also binds `tests/*.py`, `conftest.py`, `pyproject.toml`, the configuration file, `uv.lock` and the manifest of the corpus used. After a validation is published, changing any of these prevents resuming and forces a new directory with V2 and V3 redone. The documents stay outside the perimeter and can be changed; `archive/` is outside the perimeter but bound to the bytes of `5f1ec06` by `test_every_archived_file_has_its_bytes_at_the_base`. The realignment and the rename changed the code identity **before** any V2 or V3 evidence was published: at that time, under `results/hexis31/` there were only the V1 runs. The tests added by V3-005 are such a change after publication: the directories of 24 September are not resumed, and V2 and seed 0 are rerun in new ones. The fixes of V3-006 are another: the directories of V3-005 are not resumed either. V3-007 changes only the sdist list of `pyproject.toml`, after V5: the code identity and run ID do not change, but `--resume` and `run_report` refuse `v3-seed0-v3006` on `master`; the evidence runs from the tag `hexis31-v5-evidence`, while `validate_run` keeps accepting it everywhere.
...
5. **English companions (V3-003)**: done on 25 September 2026 in four tranches, in `docs/contracts/hexis-3.1-en/`, outside the deposit root and inside the `docs/contracts` entry of the sdist, so the frozen `pyproject.toml` is unchanged. Seventeen non-normative translations: the plan 3.1, the V3-001 and V3-002 acts (`decision-log/`), the consolidated verdict and its four evidence reports, the minor texts of the attachments, the two texts found only inside ZIPs (mirrored with the archive name as a folder) and the complete plans 3.0.1 and 3.0. Each opens with "the Italian original governs" and the SHA-256 of its original; code and JSON blocks are reproduced unchanged; translator's notes are marked. A check kept outside the repository and published by V3-007 as [check_companions.py](contracts/hexis-3.1-en/check_companions.py), with its [pairs](contracts/hexis-3.1-en/pairs_all.json), compares header hash, headings, table rows, formulas, code blocks, lists and every number with the original: 17/17 OK. No test was added, since tests are in the freeze perimeter.
6. **V4–V5**: campaign executed on 24 September 2026 and report on 25 September, in `results/hexis31/v3-seed0-v3006`; results and limitations stated and V5 completed on 25 September ([RESULTS.md](RESULTS.md), until V3-007 the first section of this handoff), README updated (§17.2). The act for the publication of the C0 position vectors and other corpus derivatives (§17.3) is V3-007, 25 September 2026.
