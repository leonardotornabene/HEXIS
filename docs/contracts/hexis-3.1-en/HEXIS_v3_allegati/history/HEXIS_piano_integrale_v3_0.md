...
> Original: `docs/contracts/hexis-3.1/HEXIS_v3_allegati/history/HEXIS_piano_integrale_v3_0.md` (historical plan 3.0, superseded by 3.0.1 and then by 3.1), SHA-256 `b4cf25f17e011f9a4d6798a39fac103c314702e4297ee5d4232662853937910b`.
> Translated on 2026-09-25. 245 of its 259 paragraphs are identical to plan 3.0.1 and reuse that companion's translation; the 14 that differ are translated here. Structure, numbering, tables, formulas, values and identifiers follow the original; numbers in the prose use English notation. Any translator's note is marked *[Translator's note: …]* and changes nothing in the original.

# HEXIS 3.0 — Complete update plan

**Sequential organization of morphosyntactic annotations in a finite corpus of ancient Greek**  
**Scientific specification, operational protocol and migration plan — 11 September 2026**
...
| T20 | Aggregation from losses → documents → blocks → groups; weights 1/2 and 1/5 | Fixtures with documents of different lengths and known answers |
...
| T28 | No import of `candidates/` or of the retired protocols; no skipped test-gate | Static check and executable inventory |
...
| T30 | Test suite, documents, configuration and CLI agree on the v3 perimeter | Migration gate and final check |
...
Create the implementation branch from the verified revision, preserving the Git history. Record a decision **V3-001** in a new namespace, so as not to collide with D55 and its technical acts. V3-001 entirely replaces the v2.1 operational design and the A/B proposals for the v3 executions. D=01–D54 and the G1/D55 acts remain historical documentation; none of their methodological clauses survives implicitly in conflict with this plan.

*[Translator's note: «D=01–D54» is a typo of the original for D01–D54, corrected in revision 3.0.1 (see `CHANGELOG_REVIEW_2026-09-12.md`); it is reproduced here unchanged.]*
...
| `src/hexis/pipeline/run_confirmatory.py` | Retire from the active path; the new `run_descriptive.py` runs pairs and aggregations |
...
| `stats/permutation.py`, `bootstrap.py`, `holm.py` | No use in the v3 pipeline; keep in the Git history, remove active exports/references |
...
|---|---|---|
| V0 — Migration of the contract | Deposit V3-001, documents, schema, registry and test inventory | A single v3 authority, no retired parameter/entry point still mandatory |
...
`run_descriptive` runs both arms and the probes, not only the original. `run_sensitivity` runs all eight cells in the order of the JSON, each with both arms. The report refuses to present a final conclusion if the contract requires missing artifacts. `--resume` is an explicit option of the stages that produce results, with the rules of §11.6.
...
The environment of the first audit of the canonical suite used the repository lock and NumPy 2.5.1; the environment of the prototype and of the additional contracts uses NumPy 2.3.5. They are two distinct environments and must not be confused in a single reproducibility attestation.

For the integration Python 3.12 is kept and NumPy 2.3.5 and PyYAML 6.0.3 are pinned, consistently with the reference vectors of the contract. The other direct dependencies remain those already present (`pandas, pyarrow, conllu, scipy, matplotlib, pytest`); no CTW library is added. The lock is regenerated and deposited in the V0/V2 commit, keeping where compatible the other existing resolutions, then installed with `--frozen` in V3. Resolution and installation must end without silent substitutions. The hashes of the lock and the actual versions enter the manifest; the new complete stack must pass the suite. It is not already attested as installed by the prototype trial alone.
