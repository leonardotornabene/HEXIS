# HEXIS 3.0 — Piano integrale di aggiornamento

**Organizzazione sequenziale delle annotazioni morfosintattiche in un corpus finito di greco antico**  
**Specifica scientifica, protocollo operativo e piano di migrazione — 11 settembre 2026**
...
| T20 | Aggregazione da perdite → documenti → blocchi → gruppi; pesi 1/2 e 1/5 | Fixture con documenti di lunghezze diverse e risposte note |
...
| T28 | Nessun import di `candidates/` o dei protocolli ritirati; nessun test-gate saltato | Controllo statico e inventario eseguibile |
...
| T30 | Test suite, documenti, configurazione e CLI concordano sul perimetro v3 | Gate della migrazione e verifica finale |
...
| `src/hexis/pipeline/run_confirmatory.py` | Ritirare dal percorso attivo; nuovo `run_descriptive.py` esegue coppie e aggregazioni |
...
| `stats/permutation.py`, `bootstrap.py`, `holm.py` | Nessun uso nella pipeline v3; conservare nella storia Git, eliminare export/richiami attivi |
...
| Permutazioni delle etichette, inclusi ranghi sulle21allocazioni | Ritirate; nessuna lettura surrettizia come evidenza di significatività |
...
|---|---|---|
| V0 — Migrazione del contratto | Depositare V3-001, documenti, schema, registro e inventario test | Un’unica autorità v3, nessun parametro/entry point ritirato ancora obbligatorio |
...
| V3 — Freeze dell’esecuzione | Prova integrata a un seme per tutte le9 celle, controllo risorse, lock e manifest | 126 fit tecnici completati,84 valutazioni probe C0; nessuna modifica metodologica successiva senza versione nuova |
...
La prova V3 non è una fase per scegliere iperparametri sui contrasti. Serve a verificare l’integrazione di tutte le celle. Si controllano punteggi finiti, identità e risorse; si evita di calcolare il contrasto tra gruppi prima del freeze operativo, pur riconoscendo che pilot precedenti sono già stati osservati.
...
L’ambiente del primo audit della suite canonica usava il lock del repository e NumPy 2.5.1; l’ambiente del prototipo e dei contratti supplementari usa NumPy 2.3.5. Sono due ambienti distinti e non devono essere confusi in un’unica attestazione di riproducibilità.

Per l’integrazione si mantiene Python 3.12 e si fissa NumPy 2.3.5 e PyYAML 6.0.3, coerentemente con i vettori di riferimento del contratto. Le altre dipendenze dirette restano quelle già presenti (`pandas, pyarrow, conllu, scipy, matplotlib, pytest`); non si aggiunge una libreria CTW. Il lock viene rigenerato e depositato nel commit V0/V2, conservando ove compatibili le altre risoluzioni esistenti, poi installato con `--frozen` in V3. Risoluzione e installazione devono terminare senza sostituzioni silenziose. Gli hash del lock e le versioni effettive entrano nel manifest; il nuovo stack completo deve superare la suite. Non è già attestato come installato dalla sola prova del prototipo.
...
Le posizioni primarie `oth` sono 109.716, contro 109.108 C0. Con le otto sensibilità, C0 e i probe, gli output contengono **12.704.540 righe accoppiate**. Otto float64 e 12 byte di identificativi per riga richiedono circa921 MiB, prima di metadata, validità dei null e indici. La compressione può ridurre lo spazio ma non è una garanzia. Riservare **10 GB di spazio libero** per input, partizioni, temporanei e risultati; una macchina con **8 GB di RAM** è una dotazione prudente per l’esecuzione sequenziale. Non si dichiara un requisito minimo misurato di8 GB: il modello isolato usa molto meno.
