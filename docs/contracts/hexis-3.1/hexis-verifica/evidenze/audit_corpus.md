# Audit corpus indipendente, 2026-09-15

Nessun fit; lettura CoNLL-U e JSON nello ZIP. Confrontati direttamente tutti i conteggi documentali (raw, frasi, ritenzione, eleggibilità, frasi vuote e lunghezza massima) per tutte e tre le codifiche: nessuna discrepanza.
...
## Metadata upstream, fetch diretto al commit
...
## Supporto dopo accorpamento ADV_PART
...
## Interpretazione e confronto fra revisioni

1. Piano e B: tutti i conteggi documentali verificati direttamente sono esatti. A (§11, riga 471) dichiara correttamente di non aver letto i CoNLL-U: il suo precedente controllo era interno al pacchetto e non indipendente dai conteggi attesi.
2. B: confermati 11.270 PART nel gruppo HEX e zero nell'intera prosa; le frequenze di γάρ per blocco coincidono. Precisazione importante: Inno a Demetra ha zero PART e tutti i suoi 13 γάρ sono ADV; gli altri 526 γάρ del blocco omerico sono PART nell'Iliade. Quindi la presenza di PART non coincide perfettamente con HEX a livello documentale.
3. Accorpare globalmente PART/ADV in ADV_PART mantiene ritenzione ed eleggibilità; gli alfabeti completi diventano 100/105/11. I simboli esclusivi dei probe sono 10/12/0 prima dell'accorpamento e 9/10/0 dopo. PART:nmod perde l'esclusività quando confluisce in ADV_PART:nmod, già presente nei primari via ADV:nmod; in oth anche ADV:oth e PART:oth si fondono in un tipo. Eliminare i punteggi dei probe senza cambiare l'inventario lascia quindi 9 simboli C0 esclusivamente derivati dai sei testi non valutati. È una convenzione del supporto esplicita, non una nuova indipendenza del test.
4. Accorpamento circoscritto, non uniformazione: δέ e τε differiscono anche fra ADV/PART e CCONJ, e fra cc/advmod. Né upos_only né oth correggono tutte le differenze. Non si può attribuire il segno o l'entità di Q a questo problema senza nuova analisi, qui non eseguita. A a=0,5 cambia anche la massa Dirichlet totale: C0 53→50; oth 56→52,5; UPOS 6→5,5.
5. Correzione fattuale ad A §7.2: nei file fissati sono presenti 27 relazioni base, 28 etichette con nsubj:outer (non 29+1). Tutti i 374 token discourse sono INTJ, eliminati già dal filtro UPOS; oth non li recupera. Fra gli UPOS ammessi si scartano 993 vocative e 2 orphan nell'intero corpus; HEX perde 485 vocative (459 omerico, 26 esiodeo). Non descrivere oth come controllo delle particelle discourse.
6. A §7.1: PROPN e SYM sono effettivamente assenti. La mappatura è difensiva e non una normalizzazione osservata. L'asserzione dedicata può documentare questo fatto; l'hash già blocca qualunque mutazione dei file.
7. A riga 241: subdoc non fornisce la marcatura di citazioni richiesta dalla raccomandazione. Nei due XML upstream di Ateneo i valori sono riferimenti di passo (esempi nel JSON) e gli elementi disponibili non includono quote/q/cit. Non segue che citazioni in versi siano assenti: segue che censirle richiederebbe un diverso dato o giudizio editoriale. Per un progetto ridotto: nessuna nuova annotazione manuale obbligatoria; esplicitare la composizione e il peso di Ateneo (16.691/56.099 = 29,75% dei target prosastici).
8. B: quote eleggibili confermate: HEX 53.009/80.370 = 65,9562%; prosa 56.099/73.930 = 75,8812%. Il filtro dei primi quattro simboli è distinto dalla ritenzione; la diagnostica per fasce di storia non rende uguali le distribuzioni complete.
9. Bound: il piano §7.2 già specifica che il contatore intrafrastico riparte da zero e che le coordinate eleggibili sono identiche a C0. È utile rendere esplicito il medesimo collegamento sul test. Affermazioni di inerzia non seguono dalla sola geometria del contesto: a D8, j4/j5/j6 possono vedere token oltre SEP, j7 soltanto SEP; i pesi possono variare anche altrove. Non ho rieseguito campionamenti o fit bound: le percentuali di collegamento di B restano un'attestazione della revisione, non un nuovo risultato di questo audit.

## Fonti primarie verificate

- https://universaldependencies.org/treebanks/grc_perseus/index.html (documentazione UD: conversione automatica, assenza PROPN/SYM, totali).
- https://github.com/PerseusDL/treebank_data/blob/bf4334f0af5e13d16b04c1cccd6237e683ac6f5f/v2.1/Greek/README.MD#L67 (contenuto recuperato direttamente al commit: la distinzione adverb/particle richiede ancora lavoro; browser mostra anche la medesima nota in master).
...
Solo i due XML Ateneo sono stati nuovamente ispezionati in questo audit. La dichiarazione B che tutti i 18 XML espongano guidelines 1.1 non è stata nuovamente verificata su tutti i 18; è corretta per i due qui letti.

## Riproduzione

JSON consolidato: `/private/tmp/hexis_corpus_audit.json`.
Script indipendente stdlib: `/private/tmp/hexis_corpus_recount.py`.
Comando: `python3 /private/tmp/hexis_corpus_recount.py --repo /Users/leonardoTornabene/Projects/hexis --zip /Users/leonardoTornabene/Desktop/HEXIS_allegati_operativi_v3_2026-09-11.zip`.
Lo script ricalcola censimento e supporti e asserisce i conteggi attesi; non importa modelli né legge candidates. Lo script riflette il codice eseguito nei conteggi della sessione ed è stato controllato sintatticamente; non è stato rieseguito integralmente dopo l'istruzione di non ripetere il censimento. Il JSON esporta i risultati già ottenuti.
