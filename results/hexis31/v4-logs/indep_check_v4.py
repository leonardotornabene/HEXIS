"""V4 variant (all seeds, 490 pairs) of the recomputation of a seed-0 run from the deposited contract, without importing hormathos.
...
def check_run(run, corpus):
    ...
    pairs = sorted(run.glob('pair__*.json'))
    check('pair count', len(pairs) == DESIGN['expected']['model_pairs'], len(pairs))
    ...
    check('cells', dict(per_cell) == {c: 7 * len(cells[c]['seeds']) for c in cells}, dict(per_cell))
    # §14.2(3): pair and model keys generated from the contract, set equality after a duplicate check
    blocks = {b['block']: b['block_key'] for b in DESIGN['blocks']}
    want_pairs = sorted((c, k, s) for c in cells for k in blocks.values() for s in cells[c]['seeds'])
    got_pairs = [tuple(x) for x in manifest['keys']['pair']]
    check('pair keys no duplicates', len(got_pairs) == len(set(got_pairs)), len(got_pairs))
    check('pair keys == contract', sorted(got_pairs) == want_pairs, len(got_pairs))
    want_models = sorted((c, k, s, a) for c, k, s in want_pairs for a in cells[c]['arms'])
    got_models = [tuple(x) for x in manifest['keys']['model']]
    check('model keys no duplicates', len(got_models) == len(set(got_models)), len(got_models))
    check('model keys == contract', sorted(got_models) == want_models, len(got_models))
    on_disk = {p.name for p in run.iterdir()}
    check('files == manifest artifacts + manifest', on_disk == set(manifest['artifacts']) | {'manifest.json'},
          sorted(on_disk ^ (set(manifest['artifacts']) | {'manifest.json'}))[:5])
    check('ledger files', ledgers_seen == {p.name for p in run.glob('sample_ledger__*.json')},
          len(ledgers_seen))
    check('C0 position rows', c0_rows == DESIGN['expected']['C0_paired_position_rows'], c0_rows)
    for cell in sorted(eligible_by_cell):
        variant = cells[cell]['variant']
        check(f'eligible pairs {cell}', eligible_by_cell[cell] == DESIGN['expected']['primary_eligible'][variant] * len(cells[cell]['seeds']),
