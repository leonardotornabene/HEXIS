...
def corpus_annotation(documents, contingency, variant, run_id) -> Figure:
    ...
    frame = frame.sort_values(['primary', 'dependence_block', 'doc_id'], ascending=[False, True, True])
    ...
    discontinuity.set(xlabel='share of raw tokens', title='Source PART and ADV (merged: ADV_PART)')
...
def block_document_profiles(block_pairs, document_scores, seed_summaries, run_id) -> Figure:
    ...
    documents = scores.ce_gain_q(scores.roll_up(
    ...
    for row, (level, frame) in enumerate((('block', blocks), ('document', documents))):
        ...
        for column, (metric, name) in enumerate(PROFILE):
            ...
            axis.set_xticks(np.arange(len(units)), units, rotation=60, ha='right', fontsize=7)
...
def sensitivities_two_weights(contrasts, sensitivity_pairs, seed_summaries, run_id) -> Figure:
    """Figure 3: D_Q under both weightings, the paired differences on the same C0 seeds, bands."""
    cells = [REFERENCE_CELL, *sorted(set(contrasts['cell']) - {REFERENCE_CELL})]
...
def R1(jsd_pairs, jsd_centroids, jsd_contributions, variant, run_id) -> Figure:  # the contract's name
    ...
    image = matrix_axis.imshow(matrix, cmap='viridis', vmin=0.0)
    ...
    centroid_axis.set(ylabel='JSD (bits)', title='Equal-block group centroids: HEX vs PROSE_ALL')
    for column, variant in enumerate(variants):
        ...
        axis.set(xlabel='symbol id (all symbols)', ylabel='contribution (bits)',
...
def supports_mixture_masses(model_diagnostics, arm_diagnostics, run_id) -> Figure:
    ...
    arms = arm_diagnostics[arm_diagnostics['cell'].eq(REFERENCE_CELL)]
...
def render(tables, documents, contingency, run_id) -> dict:
    """The five figures as SVG bytes, keyed by their artifact names."""
    ...
    figures = (corpus_annotation(documents, contingency, variant, run_id),
               block_document_profiles(tables['block_pairs.csv'], tables['document_scores.csv'],
                                       tables['seed_summaries.csv'], run_id),
    ...
    with matplotlib.rc_context(SVG_SETTINGS):
        for name, figure in zip(ARTIFACTS, figures):
            buffer = io.BytesIO()
